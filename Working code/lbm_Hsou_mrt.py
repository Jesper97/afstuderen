...
def temperature(T_old, fL_old, ux, uy, t, T_dim_C, T_dim_H):
    ...
    def energy_eq(i, j, T, ux, uy, fL, fL_old):
        ...
        T_new = T[i, j] * (1 - 6 * alpha[im, jm]) + T[i, jm] * (uy[im, jm] + 2 * alpha[im, jm]) + T[i, jp] * (-uy[im, jm] + 2 * alpha[im, jm]) + \
                T[im, jm] * (-ux[im, jm] / 4 - uy[im, jm] / 4 - alpha[im, jm] / 2) + T[im, j] * (ux[im, jm] + 2 * alpha[im, jm]) + \
                T[im, jp] * (-ux[im, jm] / 4 + uy[im, jm] / 4 - alpha[im, jm] / 2) + T[ip, jm] * (ux[im, jm] / 4 - uy[im, jm] / 4 - alpha[im, jm] / 2) + \
                T[ip, j] * (-ux[im, jm] + 2 * alpha[im, jm]) + T[ip, jp] * (ux[im, jm] / 4 + uy[im, jm] / 4 - alpha[im, jm] / 2) - \
