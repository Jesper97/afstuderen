...
def fluid_temp(Nx, Ny, g_plus, g_minus, g_star):
    for i in range(1, Nx-1):
        ...
        g[i, 1:Ny-1, 1] = g_star[i-1, 1:Ny-1, 1]
        g[i, 1:Ny-1, 2] = g_star[i, 0:Ny-2, 2]
        g[i, 1:Ny-1, 3] = g_star[i+1, 1:Ny-1, 3]
        g[i, 1:Ny-1, 4] = g_star[i, 2:Ny, 4]
...
def left_wall_temp(Ny, g_plus, g_minus, g_star, w, T):
    ...
    g[i, 1:Ny-1, 0] = g_star[i, 1:Ny-1, 0]
    g[i, 1:Ny-1, 1] = -g_star[i, 1:Ny-1, 3] + 2 * w[3] * T[1:Ny-1]      # Bounce
    g[i, 1:Ny-1, 2] = g_star[i, 0:Ny-2, 2]
    g[i, 1:Ny-1, 3] = g_star[i+1, 1:Ny-1, 3]
    g[i, 1:Ny-1, 4] = g_star[i, 2:Ny, 4]
...
def lower_wall_temp(Nx, g_plus, g_minus, g_star):
    ...
    g[1:Nx-1, j, 2] = g_star[1:Nx-1, j, 4]      # Bounce
    g[1:Nx-1, j, 3] = g_star[2:Nx, j, 3]
    g[1:Nx-1, j, 4] = g_star[1:Nx-1, j+1, 4]
...
def lower_left_corner_temp(g_plus, g_minus, g_star, w, T):
    ...
    g[i, j, 0] = g_star[i, j, 0]
    g[i, j, 1] = -g_star[i, j, 3] + 2 * w[3] * T[j]
    g[i, j, 2] = g_star[i, j, 4]      # Bounce
    g[i, j, 3] = g_star[i+1, j, 3]
    g[i, j, 4] = g_star[i, j+1, 4]
...
def lower_right_corner_temp(Nx, g_plus, g_minus, g_star, w, T):
    ...
    g[i, j, 0] = g_star[i, j, 0]
    g[i, j, 1] = g_star[i-1, j, 1]
    g[i, j, 2] = g_star[i, j, 4]      # Bounce
    g[i, j, 3] = -g_star[i, j, 1] + 2 * w[1] * T[j]
    g[i, j, 4] = g_star[i, j+1, 4]
...
def upper_left_corner_temp(Ny, g_plus, g_minus, g_star, w, T):
    ...
    g[i, j, 0] = g_star[i, j, 0]
    g[i, j, 1] = -g_star[i, j, 3] + 2 * w[3] * T[j]
    g[i, j, 2] = g_star[i, j-1, 2]      # Bounce
    g[i, j, 3] = g_star[i+1, j, 3]
    g[i, j, 4] = g_star[i, j, 2]
