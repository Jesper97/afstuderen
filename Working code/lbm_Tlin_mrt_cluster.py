...
# CSV filenames
path_name = f"/home/jhsvanwinden/Working code/Figures/Tlin/{material}/Ra108/N180/"
suffix = f"Ra{np.format_float_scientific(Ra, precision=3)}_Pr{np.round(Pr, 3)}_Ste{np.round(Ste, 3)}_tau{tau}_N={Nx}x{Ny}.png"
csv_path = f"/home/jhsvanwinden/Working code/sim_data/Tlin/{material}/Ra108/N180/"
...
def outputs(f_str, T, fL, B, Nu, FoSte, t):
    ...
    plt.title(f'Octadecane \n $f_L$, left wall at $T={np.round(TH_phys, 1)}K$, $t={np.int32(t/Nt*Time)}s$')
    plt.colorbar()
    plt.savefig(path_name + f"heatmap_fl_t={np.int32(t/Nt*Time)}" + suffix)
...
def solve(h, capp, fL, B):
    ...
    for t in range(Nt):
        ...
        if (t > 5000) and (t % np.int32(Nt / 1000) == 0):
