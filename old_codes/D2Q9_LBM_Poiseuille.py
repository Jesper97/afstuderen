...
lx = 1280  # Lattice points in x-direction
ly = 80  # Lattice points in y-direction
...
tau = 0.9  #(3. * nulb + 0.5)  # relaxation parameter
cylinder = False
...
F = 0.0001 * w_i * np.array([0, 1, 0, -1, 0, 1, -1, -1, 1])

## Functions: ##
# Equilibrium distribution function.
@jit
def equilibrium(rho, max_x, max_y, directions, velo_x, velo_y, c, F):
    ...
    for i in range(directions):
        feq[:, :, i] = rho * w_i[i] * (1 + uc[:, :, i] + 0.5 * uc2[:, :, i] - u2) + F[i]
...
f_i = equilibrium(rho, lx, ly, q, ux, uy, e_i, F)
f_eq = np.copy(f_i)
...
for t in range(maxIter):
    ...
    f_eq = equilibrium(rho, lx, ly, q, ux, uy, e_i, F)
    ...
    if (t == 998):
        r = np.linspace(-ly/2, ly/2, num=ly)
        r_phys = r
        R = r_phys[-1]
        u_max = np.amax(ux[15, :])
        u_th = u_max * (1 - r_phys**2 / R**2)

        plt.plot(ux[15, :], r_phys)
        ...
        plt.colorbar()
        plt.savefig("/Users/Jesper/Documents/MEP/Code/Figures/Pois_test/velx_" + str(t / 100) + ".png")

        plt.clf()
        plt.imshow(uy.transpose(), cmap=cm.Blues)
        plt.colorbar()
        plt.savefig("/Users/Jesper/Documents/MEP/Code/Figures/Pois_test/vely_" + str(t / 100) + ".png")


        plt.clf()
        plt.imshow((np.sqrt(ux ** 2 + uy ** 2)).transpose(), cmap=cm.Blues)
        plt.colorbar()
        plt.savefig("/Users/Jesper/Documents/MEP/Code/Figures/Pois_test/vel_" + str(t / 100) + ".png")
